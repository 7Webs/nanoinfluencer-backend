...
export class DealsService {
  ...
  async update(
    ...
    if (deal.shopId != user.owen.id) throw new UnauthorizedException();
  ...
  async topDeals(userId: string | undefined, pagination: DealSearchDto) {
    ...
    if (categoryId)
      query.andWhere('shop.categoryId = :categoryId', { categoryId });
  ...
  async getDealsByShop(
    ...
    if (q?.length) {
      queryBuilder.andWhere(
        new Brackets((qb) => {
          qb.where('deal.title ~* :query ', { query: q }).orWhere(
            'deal.keywords ~* :query ',
            { query: q },
          );
        }),
